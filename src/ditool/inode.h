...
struct 	icommon
{
    uint16_t	ic_mode;	/*  0: mode and type of file */
    int16_t	    ic_nlink;	/*  2: number of links to file */
    uint16_t	ic_uid;		/*  4: owner's user id */
...
/* inode block */
struct    inb {
    struct icommon ibs[MAXBSIZE / sizeof(struct icommon)];
};
/* indirection block */
struct    idb {
