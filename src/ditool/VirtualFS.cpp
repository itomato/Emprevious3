...
#define ACCESSPERMS (S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP | S_IROTH | S_IWOTH)

#include <sys/stat.h> // For ACCESSPERMS
...
int setxattr(const char *path, const char *name, const void *value, size_t size, int flags) {
    // Provide an appropriate implementation or return an error code
    return -1;
}
...
#else
	    if(::setxattr(hostPath.c_str(), NFSD_ATTRS.c_str(), serialized.c_str(), serialized.length(), 0) != 0)
...
#else
	    if(::getxattr(hostPath.c_str(), NFSD_ATTRS.c_str(), buffer, sizeof(buffer)) > 0)
